package com.lsx.network.calladapter

import android.util.Log
import com.lsx.network.ApiError
import com.lsx.network.ApiResult
import com.lsx.network.exception.ApiException
        ...
        delegate.enqueue(object : Callback<T> {
            ...
            override fun onResponse(call: Call<T>, response: Response<T>) {
                if (response.isSuccessful) {//http status 是200+
                    ...
                    Log.v("lsx", "body:" + response.body())
                    callback.onResponse(this@ApiResultCall, Response.success(apiResult))
                } else {//http status错误
                    val failureApiResult = if (response.errorBody() == null) {
                        ApiResult.Failure(
                        ...
                        ApiResult.Failure(ApiError.dataIsNull.errorCode,msg)
                    }
            ...
            override fun onFailure(call: Call<T>, t: Throwable) {
                val failureApiResult = when (t) {
                    is ApiException -> {//Interceptor里会通过throw ApiException 来直接结束请求 同时ApiException里会包含错误信息
                        ApiResult.Failure(t.errorCode, t.errorMsg)
                    }
                    is UnknownHostException -> {
                        ApiResult.Failure(
                            ApiError.UnknownHostException.errorCode,
                            ApiError.UnknownHostException.errorMsg
                        )
                    }
                    else -> {
                        ApiResult.Failure(
